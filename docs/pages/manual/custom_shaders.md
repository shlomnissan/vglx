...
Per-frame render state is provided through a uniform block. The resolution is the size of the render target being drawn to, or the viewport when rendering to the screen. The scale is the display's DPI scale factor, as passed to [SetViewport](/reference/core/renderer). The time is the number of seconds elapsed since the renderer was created, which is useful for animating shaders without updating a custom uniform every frame:
